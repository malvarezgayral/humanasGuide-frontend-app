...
export const UploadFile = () => {
  ...
  const handleChangeMajor = (event: any) => {
    if (carreras) {
      const selectedMajor = carreras.find(
        (major: { id: number }) => major.id === Number(event.target.value)
      );
      if (selectedMajor) {
  ...
  const handleChangeSubject = (event: any) => {
    const selectedSubject = catedras.find(
      (subject: { id: number }) => subject.id === Number(event.target.value)
    );
    if (selectedSubject) {
  ...
  const handleSubmit = async () => {
    ...
    if (
      typeof isFormValid === "function" ? isFormValid() : Boolean(isFormValid)
    ) {
          ...
          {!isLoading && (
            <>
              ...
              <CustomSelection
                ...
                disable={
                  pickedTypeValue === "RESUMEN" ||
                  pickedSubjectValue.name === ""
                }
                errorMessage={errorMessage}
              />
              <CustomSelection
